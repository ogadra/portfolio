...
End-to-end workflow for authoring and maintaining Playwright tests with `playwright-cli`. Each section below stands on its own:

- **Planning**: explore the app and produce a spec file describing what to test.
- **Generate**: turn a spec into Playwright test files. Update the spec if it's vague or stale.
- **Heal**: diagnose failing tests, fix the code, reconcile the spec with reality.

All three sections rely on the same mechanic: run `npx playwright test --debug=cli` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics and [test-generation.md](test-generation.md) for how each `playwright-cli` action emits Playwright TypeScript.
...
A **seed test** puts the page into the state every scenario starts from: navigation, login, feature flags, and so on. Scenarios assume a fresh start _after_ the seed. `--debug=cli` pauses _inside_ this test, so every planning and generation session begins here.
...
For each target scenario, in sequence (never in parallel, since scenarios share the seed session):
...
Walk each `Steps:` entry with `playwright-cli`. Treat the spec as the plan and the live app as the source of truth. If a step is vague ("click the button": which button?), references an element that no longer exists, or contradicts the app's behaviour, update the spec to match what the app really does, then keep going. Editing the spec mid-generation is expected.
...
Record the list of failing `<file>:<line>` entries and process them one at a time. Do not fix in parallel: shared state and the single CLI session make that fragile.
...
The test starts paused. Step forward to just before the failing action or assertion, then diagnose:
